...
contract_to_mock = {
    "loan_token": MockLoanToken,
    "a_nft": MockANFT,
    "b_nft": MockBNFT,
    "loan_token_price_feed": MockV3Aggregator,
    "simple_nft_price_feed": MockV3Aggregator,
    "a_nft_price_feed": MockV3Aggregator,
    "b_nft_price_feed": MockV3Aggregator,
}
...
def get_contract(contract_name):
    ...
    if network.show_active() in NON_FORKED_LOCAL_BLOCKCHAIN_ENVIRONMENTS:
        if (
            len(contract_type)
            <= 0
            # considering contract_type
            # as an array not a string will help to understand
        ):  # deploy mocks when first time of getting mock contract
            deploy_mocks()
        contract = contract_type[-1]  # for same type just use the recent one
    else:
        try:
...
def deploy_mocks(decimals=DECIMALS, initial_value=INITIAL_VALUE):
    ...
    print("Deploying Mock Loan Token...")
    loan_token = MockLoanToken.deploy({"from": account})
    print(f"Deployed to {loan_token.address}")

    print("Deploying Mock ANFT...")
    a_nft = MockANFT.deploy({"from": account})
    print(f"Deployed to {a_nft.address}")

    print("Deploying Mock BNFT...")
    b_nft = MockBNFT.deploy({"from": account})
    print(f"Deployed to {b_nft.address}")
